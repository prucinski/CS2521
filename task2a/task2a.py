...
def findSequence(lengthOfStrand, strand1, strand2, passToNext = 0):
    ...
    if lengthOfSubstring == 0:
        return ""
    substrings1 = set(findSubstrings(strand1, lengthOfSubstring))    #argument substringList =[]  for whatever reason returns to global memory
    substrings2 = set(findSubstrings(strand2, lengthOfSubstring))    
    commonSet = substrings1 & substrings2                                               
    if len(commonSet) > 0:
